...
class TopLiquidDataLoader(AbstractDimDataLoader):
    ...
    def __init__(self, user_name, project_name, data_loader_name, universe, version_number, required_dims, new_dims,
                 ...
                 option=False, MinVol=50000, MaxVol=1 * math.exp(20), MinPrice=1, MaxPrice=1000,
                 MinCap=10000000, NoST = True, Cyb = True, FilterSuspend = False):
        ...
        else: self.required_dims = ["TURNOVERVALUE", "SPECIALTRADETYPE", "CLOSEPRICE", "TOTALSHARES"]
    ...
    def do_load(self):
        ...
        # 参数设置
        requiredpara = [self.WindowDays,self.Lookback,self.Passrate,self.Sticky,self.Delta,self.t,self.Cyb,self.FilterSuspend]
        ...
        signature = TopLiquidSignature(self.universe.start_date, self.universe.end_date, self.universe.back_days, self.universe.end_days,requiredpara,self.option,optionpara)
    ...
    def _calculate_new_dim_data(self,required_dim_datas,dim_datas):
        ...
        for di, current_date in enumerate(self.universe.date_list):
            ...
            Inst = 0
            if di >= self.WindowDays - 1:
                filter = np.zeros(len(self.universe.secu_code_list), dtype='bool') # 初始化
                # 六个限制条件筛选
                if self.option:
                    for ii, secu_code in self.universe.secu_code_list:
                        self.valid[di][ii] = False
                        sumclose = 0; sumvol = 0; sumshare = 0; ndays = 0
                        d = di + 1 - self.WindowDays
                        for dd in range(d,di + 1):
                            if np.isfinite(self.dim_data["CLOSEPRICE"][dd][ii]) & \
                                    np.isfinite(self.dim_data["TURNOVERVALUE"][dd][ii]) & np.isfinite(self.dim_data["TOTALSHARES"][dd][ii]):
                                sumclose = sumclose + self.dim_data["CLOSEPRICE"][dd][ii]
                                sumvol = sumvol + self.dim_data["TURNOVERVALUE"][dd][ii]
                                sumshare = sumshare + self.dim_data["TOTALSHARES"][dd][ii]
                                ndays = ndays + 1
                        #check for minvol, minprice, maxprice, mincap
                        if ndays > 0:
                            if ((sumclose / ndays < self.MinPrice) | (sumvol / ndays < self.MinVol) | (sumshare / ndays < self.MinCap)\
                                    | (sumclose / ndays > self.MaxPrice) | (self.dim_data["SPECIALTRADETYPE"][di - 1][ii] != 0 & self.NoST)):
                                filter[ii] = True
                                self.valid[di][ii] = False
                                outmark[ii] = False
                                outporale[ii] = self.Sticky
                        else: filter[ii] = True; self.valid[di][ii] = False; outmark[ii] = False;outporale[ii] = self.Sticky
...
if __name__ == "__main__":
    ...
    dims_to_load = ["TURNOVERVALUE", "SPECIALTRADETYPE", "CLOSEPRICE", "TOTALSHARES","TOTALFLOATSHARES"]
    ...
    sample_ip_data_loader = TopLiquidDataLoader(user_name, project_name, "1500topliquid", universe, version_number, required_dims=["TURNOVERVALUE"], new_dims=["TOP_INSTRUMENT_POOL"])
